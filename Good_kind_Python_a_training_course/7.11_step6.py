...
(исходную строку перевести в нижний регистр - малые буквы). Все небуквенные символы ": ;.,_" превращать в символ '-'
(дефиса).
...
def hyp_del(func: callable) -> callable:
    def wrapper(s: str) -> str:
        return sub(r'--+', repl='-', string=func(s))
    return wrapper


@hyp_del
def get_translate(s: str) -> str:
    return ''.join(t.get(c, c) if c not in ": ;.,_" else '-' for c in s.lower())


print(get_translate(input()))
